	...
	overlaps: function( other ){
		if ( 
			...
			(other.getStart() <= this.start && other.getEnd() >= this.end) ) {
				for( var i = 0; i < this.days.length; i++ ) { if ( other.getDays().contains( this.days[i] ) ) return true; } 
		}
		else { return false; }
	}
});
...
var Calendar = new Class({
    initialize: function(){
		...
		addSections: function( sections ){
			if ( sections.length == 1 ) { this.addSection( sections[0] ); }
				...
				for( var i = 0; i < sections.length; i++ ){
					...
					var added = false;
					for( var j = 0; j < section_groups.length; j++ ){ if ( section_groups[j].contains( section ) ) { section_groups[j].addSection( section ); added = true; }}
					if ( !added ) {
		...
		build: function(){
			...
			for( var i = 0; i < this.options.num_hours; i++ ) {
				// build guide
